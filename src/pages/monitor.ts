...
import { PlumChartData, PlumChartOptions, PointEvent, RangeEvent } from "@/core/plum-chart.types";
    ...
    const newData: Partial<PlumChartData> = {
        entities: data.entities.filter((value, idx) => {
            const eventExist = value.rangeEvents.some(operation => {
                return operation.startTime.getTime() < options.chartStartTime!.getTime() &&
                    operation.endTime.getTime() > options.chartEndTime!.getTime();
            });
            return eventExist;
        })
    }
